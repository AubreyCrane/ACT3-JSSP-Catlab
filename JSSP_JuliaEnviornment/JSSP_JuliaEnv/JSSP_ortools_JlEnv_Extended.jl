...
function parse_jssp_data(data::String)
    ...
    for i in 1:num_jobs
        ...
        for j in 1:2:length(values)
            machine_id = values[j]  # Do not adjust for 1-based indexing
            duration = values[j + 1]
            if machine_id < 0 || machine_id >= num_machines
                error("Invalid machine_id $(machine_id) for job $(i-1). Must be between 0 and $(num_machines-1).")
            end
...
function create_disjunctive_graph(jssp::JobShopProblem)
    ...
    for job_id in 1:jssp.num_jobs
        for (task_idx, task) in enumerate(jssp.jobs[job_id])
            ...
            push!(machine_tasks[task.machine_id + 1], (job_id - 1, task_idx - 1))
            v += 1
        end
    end
    ...
    for machine_id in 1:jssp.num_machines
        ...
        for i in 1:length(tasks)
            for j in i+1:length(tasks)
                ...
                push!(disjunctive_arcs, (v1, v2, machine_id - 1))
                push!(disjunctive_arcs, (v2, v1, machine_id - 1))  # Both directions
            end
        end
    end
...
function visualize_graph!(ax, g, vertex_labels, conjunctive_arcs, oriented_arcs, weights, vertex_positions, disjunctive_arcs, title="Disjunctive Graph")
    num_edges = ne(g)
    ...
    # Define color map for machines (e.g., 4 distinct colors for up to 4 machines)
    machine_colors = [RGB(1, 0, 0), RGB(0, 1, 0), RGB(0, 0, 1), RGB(1, 0.5, 0)]  # Red, Green, Blue, Orange
    ...
    for (src, dst, machine_id) in disjunctive_arcs
        if haskey(edge_index, (src, dst))
            i = edge_index[(src, dst)]
            color = machine_colors[mod1(machine_id, length(machine_colors))]  # Color by machine
            edge_colors[i] = color
            edge_styles[i] = (src, dst) in oriented_arcs ? :solid : :dot  # Solid if oriented, dotted if not
        end
    end
...
function solve_jssp(jssp::JobShopProblem)
    ...
    # Initial plot
    visualize_graph!(ax, g, vertex_labels, conjunctive_arcs, [], vertex_weights, vertex_positions, disjunctive_arcs, "Initial Graph (No Disjunctive Arcs Oriented)")
    ...
    record(f, "jssp_optimization_exact.gif", Iterators.product([permutations(1:length(machine_tasks[m])) for m in 1:jssp.num_machines]...); framerate=2) do machine_perms
        ...
        for machine_id in 1:jssp.num_machines
            ...
            for i in 1:length(ordered_tasks)-1
                ...
                visualize_graph!(ax, oriented_g, vertex_labels, conjunctive_arcs, temp_arcs, vertex_weights, vertex_positions, disjunctive_arcs, title)
                yield()
            end
        end
        ...
        visualize_graph!(ax, oriented_g, vertex_labels, conjunctive_arcs, oriented_arcs, vertex_weights, vertex_positions, disjunctive_arcs, title)
        yield()
    end
...
# Example usage with an expanded instance (5 jobs, 4 machines)
jssp_data = """
instance expanded_jobshop_example
Expanded instance of a job-shop problem
5 4
0 3 1 2 2 3
0 2 2 3 3 1
1 1 2 3 3 2
